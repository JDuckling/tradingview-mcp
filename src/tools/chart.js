...
export function registerChartTools(server) {
  ...
  }, wrapOk('chart_set_timeframe', ({ timeframe }) => core.setTimeframe({ timeframe })));
  ...
  }, wrapOk('chart_set_type', ({ chart_type }) => core.setType({ chart_type })));
  ...
  }, wrapOk('chart_manage_indicator', (args) => core.manageIndicator(args)));

  server.tool('chart_get_visible_range',
    'Get the visible date range (unix timestamps) and bars range on the chart',
    {},
    wrapOk('chart_get_visible_range', () => core.getVisibleRange()));
  ...
  }, wrapOk('chart_set_visible_range', ({ from, to }) => core.setVisibleRange({ from, to })));
  ...
  }, wrapOk('symbol_search', ({ query, type }) => core.symbolSearch({ query, type })));
}
