...
> **Fork notice:** This is the AlphaSignal-maintained fork of `tradesdontlie/tradingview-mcp`. Upstream maintainer has been inactive since 2026-04-04, so we carry our own patches ahead of upstream. Bugs closed in this fork (not yet merged upstream): **#140** (`quote_get` / `data_get_ohlcv` ignore symbol), **#171** (`evaluate is not defined`), **#116 + #137** (drawing API broken), **#143** (`data_get_study_values` same-name dedup), **#144** (`capture_screenshot` stale frame), **#142** (stuck state after failed indicator add), **#164** (`watchlist_add` button not found), plus a TV 3.1.0 Pine Editor selector drift fix. Also includes a v3.x unified `OperationResult` response contract across all 78 tools. See [`docs/MAINTENANCE.md`](docs/MAINTENANCE.md) for the patches log, quality gates, two smoke variants (`npm run smoke` + `npm run smoke:mcp`), failure-log rotation, and upstream-sync procedure.
